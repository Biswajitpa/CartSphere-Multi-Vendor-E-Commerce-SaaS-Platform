...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      if (paymentMode === "cod") {
        // ✅ FIXED: was router.replace("/orders") — now goes to order-success
        await axios.post("/api/order/create-cod", payload);
        router.replace("/order-success?payment=cod");
      } else {
        ...
        const options = {
          ...
          handler: async (response: any) => {
            try {
              ...
              // ✅ Online payment — also passes payment=online for consistency
              router.replace("/order-success?payment=online");
            } catch (err) {
      ...
      >
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-4 bg-white/5 p-4 rounded-xl border border-white/10">
            ...
            <p className="font-bold text-green-400">₹ {productsTotal}</p>
          </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className={`flex-1 py-3 rounded-xl font-semibold transition ${
                  paymentMode === "cod" ? "bg-blue-600" : "bg-white/10"
              ...
              <button
                ...
                className={`flex-1 py-3 rounded-xl font-semibold flex items-center justify-center gap-2 transition ${
                  paymentMode === "razorpay" ? "bg-blue-600" : "bg-white/10"
                }`}
              >
                💳 Razorpay
              </button>
